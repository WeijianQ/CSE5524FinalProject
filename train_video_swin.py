...
class CollisionLitModel(pl.LightningModule):
    def __init__(self, lr: float = 1e-4, freeze: bool = True):
        super().__init__()
        self.net = VideoSwinTiny(num_classes=1, freeze=freeze)
        self.lr = lr
        # loss and metrics
...
def main(checkpoint_path: Optional[str] = None, sample_choice: str = "end_biased", freeze: bool = True):
    # -------------- W&B --------------
    import os
    ...
    if os.getenv("DO_DEBUG") == "1":
        ...
        version = f"{config['sample_choice']}_32frm_small_lr"
        if freeze:
            version += "_frozen"
        else:
            version += "_unfrozen"
...
def test_model(checkpoint_path: str, sample_choice: str = "end_biased"):
    ...
    output_path = f"submission_{sample_choice}.csv"
...
if __name__ == "__main__":
    ...
    if args.do_test:
        ...
        main(checkpoint_path=args.resume, sample_choice=args.sample_choice, freeze=args.freeze)
